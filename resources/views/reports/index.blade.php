          ...
          <div class="card">
            <div class="card-header">
              <div class="d-flex align-items-center">
                <h4 class="card-title">Liste des reports</h4>
                <button
                  class="btn btn-primary btn-round ms-auto"
                  data-bs-toggle="modal"
                  data-bs-target="#addRowModal"
                >
                  <i class="fa fa-plus"></i> Ajouter un report
                </button>
              </div>
            </div>
            ...
            <div class="card-body">
                    ...
                    <tbody>
                        @foreach($reports as $key => $report)
                            <tr>
                                ...
                                <td>{{$report->patient->first_name}} {{$report->patient->middle_name}} {{$report->patient->last_name}}	</td>
                                ...
                                <td>
                                    <div class="form-button-action">
                                        ...
                                        <button
                                            ...
                                            data-info="{{$report->id}},{{$report->name}},{{$report->department_id}},{{$report->amount}}"
                                        >
                        ...
                        <div class="modal-content">
                            <div class="modal-header border-0">
                                <h5 class="modal-title">
                                    ...
                                    <span class="fw-light"> report</span>
                                </h5>
                            ...
                            <div class="modal-body">
                                <p class="small">Créez un nouveau report en remplissant le formulaire ci-dessous.</p>
                ...
                <div class="modal fade" id="deleteRowModal" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header border-0">
                                <h5 class="modal-title">Êtes-vous sûr de vouloir supprimer ce report ?</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
...
@section('script')
    <script type="text/javascript">
        // Événement pour modifier un report
        $(document).on('click', '.edit-button', function() {
            var reports = $(this).data('info').split(',');
            var reportId = reports[0];  // Récupérer l'ID
            var reportName = reports[1];  // Récupérer le name
            var reportDepartmentId = reports[2];  // Récupérer le departement name
            var reportAmount = reports[3];  // Récupérer le report amount
            // Mettre à jour le champ du modal
            $('#edit_id').val(reportId);
            $('#edit_name').val(reportName);
            $('#edit_department_id').val(reportDepartmentId);
            $('#edit_amount').val(reportAmount);

            // Afficher le modal
            $('#editRowModal').modal('show');
        });

        // Événement pour supprimer un report
        $(document).on('click', '.delete-button', function() {
            var id = $(this).data('id');
            var name = $(this).data('name');

            // Afficher le nom du report à supprimer
            $('#department_name_to_delete').text("Voulez-vous vraiment supprimer le report : " + name + " ?");

            // Mettre à jour l'action du formulaire de suppression avec l'ID du report
            $('#delete_id').val(id);
            $('#deleteDepartmentForm').attr('action', '/report/delete/' + id);

            // Afficher le modal de confirmation
            $('#deleteRowModal').modal('show');
        });

        // Afficher le loader pour l'ajout de report
        ...
        // Afficher le loader pour la modification de report
        ...
        // Afficher le loader pour la suppression de report
