 ...
 */
class ParcoursTest extends Command
{
    ...
    {
        // La commande doit pouvoir tourner sur une base vide : tout ce qui
        // manque est créé ici, dans la transaction annulée à la fin.
        $this->clinique = Etablissement::where('type', 'clinique')->where('statut', 'actif')->first()
            ?? $this->creerEtablissement('Clinique d\'essai', 'clinique');

        $this->laboratoire = Etablissement::where('id', '!=', $this->clinique->id)
            ->whereIn('type', ['laboratoire', 'clinique'])->first()
            ?? $this->creerEtablissement('Laboratoire d\'essai', 'laboratoire');

        $this->utilisateur = User::where('etablissement_id', $this->clinique->id)->first()
            ?? $this->creerUtilisateur('Accueil d\'essai', $this->clinique);
        Auth::login($this->utilisateur);

        $this->biologiste = User::where('etablissement_id', $this->laboratoire->id)->first()
            ?? $this->creerUtilisateur('Biologiste d\'essai', $this->laboratoire);

        $this->departement = Department::first() ?? Department::create(['name' => 'Médecine générale']);

        $this->medecin = Employee::where('type', 'Doctor')->where('is_active', true)->first()
            ?? Employee::create([
                'first_name' => 'Médecin', 'last_name' => 'Essai', 'address' => 'Conakry',
                'type' => 'Doctor', 'department_id' => $this->departement->id,
                'user_id' => $this->utilisateur->id, 'is_active' => true,
            ]);

        $this->departement = Department::findOrFail($this->medecin->department_id ?? $this->departement->id);
    ...
    private function creerEtablissement(string $nom, string $type): Etablissement
    {
        // Création hors session : connecté à un établissement, on ne peut pas
        // écrire les données d'un autre (garde-fou du cloisonnement).
        $connecte = Auth::user();
        Auth::logout();

        $etablissement = Etablissement::create([
            'nom' => $nom,
            'slug' => Str::slug($nom) . '-' . Str::lower(Str::random(4)),
            'type' => $type,
            'statut' => 'actif',
        ]);

        // Modules activés : sans eux, les écrans et les circuits restent fermés.
        foreach (['consultation', 'laboratoire', 'assurance'] as $code) {
            $module = \App\Models\Module::firstOrCreate(['code' => $code], ['nom' => Str::ucfirst($code)]);
            $etablissement->modules()->syncWithoutDetaching([$module->id => ['est_actif' => true, 'active_depuis' => now()]]);
        }

        // Catalogue d'examens du laboratoire (comme la commande labo:activer).
        if (class_exists(\App\Services\Labo\CatalogueImportService::class)) {
            try {
                app(\App\Services\Labo\CatalogueImportService::class)->importer($etablissement->id);
            } catch (Throwable $e) {
                $this->line('  <fg=yellow>Catalogue du laboratoire non importé : ' . $e->getMessage() . '</>');
            }
        }
        ...
        $this->line('  <fg=yellow>Créé pour l\'essai : ' . $nom . '</>');

        return $etablissement;
    }

    private function creerUtilisateur(string $nom, Etablissement $etablissement): User
    {
        $connecte = Auth::user();
        Auth::logout();
        ...
        $utilisateur = User::create([
            'name' => $nom,
            'email' => Str::lower(Str::random(8)) . '@essai.local',
            'phone' => (string) random_int(600000000, 699999999),
            'password' => bcrypt(Str::random(16)),
        ]);

        $utilisateur->forceFill(['etablissement_id' => $etablissement->id])->save();
